...
const DocumentList: React.FC<DocumentListProps> = ({
  ...
  const fetchDocuments = async (forceRefresh = false) => {
    ...
    try {
      setLoading(true);
      setError('');
      const response = await window.electronAPI.getCourseDocuments(selectedCourse.course_num, { skipCache: forceRefresh });
  ...
  const handleDownload = async (doc: CourseDocument) => {
    ...
    try {
      ...
      const result = await window.electronAPI.downloadCourseDocument(doc.res_url, fileName);
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2>Documents {selectedCourse ? `- ${selectedCourse.name} (${realDocuments.length})` : ''}</h2>
      ...
      {selectedCourse && (
        <div style={{ marginBottom: '20px', padding: '16px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
          ...
          <p style={{ margin: '0', color: '#666' }}>
            <strong>Teacher:</strong> {selectedCourse.teacher_name} • <strong>Course Code:</strong> {selectedCourse.course_num}
          </p>
        </div>
      )}
      ...
      {!selectedCourse ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {realDocuments.sort((a, b) => a.rpName.localeCompare(b.rpName)).map((doc) => (
            <div
              key={doc.rpId}
              style={{
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '13px', color: '#666' }}>
                  ...
                  <p style={{ margin: 0 }}>
                    <strong>Uploaded:</strong> {formatUploadTime(doc.inputTime)}
                  </p>
                  <p style={{ margin: 0 }}>
              ...
              <div style={{ marginLeft: '16px' }}>
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    backgroundColor: downloadingDoc === doc.rpId ? '#ccc' : '#28a745',
                    color: 'white',
                    border: 'none',
                    borderRadius: '4px',
                    cursor: downloadingDoc === doc.rpId ? 'not-allowed' : 'pointer',
                    fontSize: '14px'
                  }}
                >
                  {downloadingDoc === doc.rpId ? 'Downloading...' : 'Download'}
                </button>
              </div>
            </div>
          ))}
